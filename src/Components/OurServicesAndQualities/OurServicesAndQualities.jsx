...
import careMan from "../../assets/tree_care.jpg";
import serviceMan from '../../assets/woman-garden.png';
                ...
                <div className='grid py-3.5 gap-x-3 gap-y-4 sm:grid-cols-2 grid-cols-1'>
                    ...
                    <div className="flex items-center justify-center">
                        <img className={`xl:h-[29rem] sm:m-0  lg:h-[27.5rem] ${isService ? 'md:h-[20.5rem] sm:h-[17.2rem]' : 'md:h-[21rem] sm:h-[19rem]'} h-[18rem]`}
                            src={isOurQuality ? careMan : isService ? serviceMan : phone}
                            loading='lazy'
                            alt="" />
                    </div>
                            ...
                            <div className={`${isDownloadApp ? 'xl:space-y-6 space-y-4' : 'md:space-y-2.5 sm:space-y-1.5  space-y-2.5'} w-11/12`}>
                                ...
                                {isDownloadApp ?
                                    ...
                                    : <h1 className='pb-0 md:text-2xl text-xl font-semibold capitalize xl:tracking-wide'>
                                        {isOurQuality ?
                                            'Best Quality Healthy And Fresh Plant'
                                            : isService ?
                                                'Why We’re Best Indoor Plant Provider '
                                                : ''}
                                    </h1>}
                                {/* description */}
                                <p className='lg:text-base text-gray-7o 00 md:text-sm sm:text-xs text-sm'>
                                    {isOurQuality ? 'We prioritize quality in each of our plant, below are the advantage of our products. Each Plant is insect secured.' : isService ? 'We offer a wide variety of products to choose from , you can find every plant you need to decor your spaces.' : 'Our plantify mobile app makes it easy to shop for plants on the go.You can browse our selection of products, create a shopping list, and order plants for pickup or delivery.'}
                                </p>
                            </div>
